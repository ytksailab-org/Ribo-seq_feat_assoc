...
for(i in 1:length(in_files)){
  ...
  system(
    paste0(
      ...
      ' -a ',conf['cutadapt_3prime_adapter_1'],
      ' -a ',conf['cutadapt_3prime_adapter_2'],
      ' -a ',conf['cutadapt_3prime_adapter_3'],
      ' -n ',5,
      five_prime,
...
for(i in 1:length(in_files)){
  ...
  my_cmd <- paste0(
              ...
              ' ','<(zcat ',out_path,'adapterRemoval/',in_files[i],')',
  ...
  system(paste("/bin/bash -c", shQuote(my_cmd)))
  # status print
  cat('DONE \n')
}
